...
async function run(){
 ...
 for(let i=0;i<jobs.length;i++){const btn=jobs[i],label=text(btn.querySelector('b')||btn);status.textContent='Testez: '+label;document.title='QA JOB '+(i+1)+'/12 · '+label;btn.click();const out=root.querySelector('[data-cvi-out="job"]');const t=await waitFor(()=>{const x=text(out);return settledJob(x)&&x},45000,300);const state=resultState(t),hasReco=/RECOMANDARE/.test(t),logicOk=(state==='NECESITĂ VERIFICARE'||state==='INDISPONIBIL'||state==='IDENTIFICARE NECESARĂ')?!hasReco:true;jobResults.push({id:btn.dataset.cviJob,label,state,hasReco,logicOk,text:t});}
